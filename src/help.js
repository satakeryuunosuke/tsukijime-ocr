...
export const HELP_TOPICS = {
  ...
  backup_auto_folder: {
    title: "ファイルサーバー等の自動バックアップ保存先設定",
    category: "データ管理",
    badge: "📁 自動保存",
    summary: "月締め確定（完了）時に、あらかじめ指定したファイルサーバーや共有フォルダに全データバックアップ（JSON）を自動で保存します。",
    steps: [
      "PC（Google ChromeまたはMicrosoft Edge）で「データ管理」画面を開きます。",
      "「保存先フォルダを指定する」ボタンを押し、ファイルサーバーや共有ネットワークドライブ内のフォルダを選択して「保存」を許可します。",
      "月締め画面で「月締めを確定してロック」を実行すると、そのフォルダへ自動的にバックアップファイル（tsukijime_backup_YYYYMMDD.json）が書き込まれます。",
      "後から別のPCで内容を確認したい場合は、別のPC側で本システムを開き、「バックアップから復元（インポート）」からファイルサーバー内のバックアップを選択するだけで同期・確認できます。",
    ],
    tips: [
      "本機能はブラウザのFile System Access APIを使用します。Chrome / Edge（PC版）で利用可能です。",
      "一度設定すれば、同じ端末・ブラウザからの月締め完了時にワンクリックまたは自動で保存されます。",
      "「今すぐテスト保存」ボタンを押すことで、指定したフォルダへ正しく保存できるかをいつでも確認できます。",
    ],
    related: ["settings_backup", "closing_lock"],
  },
};
